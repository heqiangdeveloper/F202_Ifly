...
public abstract class CallBackUtil<T> {
    ...
    public  void onProgress(float progress, long total ,int postion,int id){}

    void onError(final RealResponse response,int position,int id){
        ...
        mMainHandler.post(new Runnable() {
            @Override
            public void run() {
                onFailure(response.code,errorMessage, position,id);
            }
        });
    }
    void onSeccess(RealResponse response,int position,int id){
        final T obj = onParseResponse(response,position,id);
        mMainHandler.post(new Runnable() {
            @Override
            public void run() {
                onResponse(obj,position,id);
            }
        });
    }
     ...
     * 解析response，执行在子线程
     */
    public abstract T onParseResponse(RealResponse response,int position,int id);

    /**
     * 访问网络失败后被调用，执行在UI线程
     */
    public abstract void onFailure(int code,String errorMessage,int position,int id);

    /**
     *
     * 访问网络成功后被调用，执行在UI线程
     */
    public abstract void onResponse(T response,int position,int id);



    public static abstract class CallBackDefault extends CallBackUtil<RealResponse> {
        @Override
        public RealResponse onParseResponse(RealResponse response,int position,int id) {
            return response;
        }
    }
    ...
    public static abstract class CallBackString extends CallBackUtil<String> {
        @Override
        public String onParseResponse(RealResponse response,int position,int id) {
            try {
    ...
    public static abstract class CallBackBitmap extends CallBackUtil<Bitmap> {
        ...
        @Override
        public Bitmap onParseResponse(RealResponse response,int position,int id) {
    ...
    public static abstract class CallBackFile extends CallBackUtil<File> {
        ...
        @Override
        public File onParseResponse(RealResponse response,int position,int id) {
            InputStream is = null;
...
//                Log.d("qlf","total ="+total);
                ...
                while ((len = is.read(buf)) != -1){
                    ...
                    mMainHandler.post(new Runnable() {
                        @Override
                        public void run() {
                            onProgress(finalSum * 100.0f / total,total,position,id);
                        }
                    });
