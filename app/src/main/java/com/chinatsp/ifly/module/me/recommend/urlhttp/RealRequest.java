...
class RealRequest {
     ...
     * 上传文件
     */
    RealResponse uploadFile(String requestURL,int position,int id, File file,List<File> fileList,Map<String,File> fileMap,String fileKey,String fileType,Map<String,String> paramsMap,Map<String, String> headerMap,CallBackUtil callBack) {
        HttpURLConnection conn = null;
        try {
            conn = getHttpURLConnection(requestURL,"POST");
            setConnection(conn);
            ...
            if(file != null) {
                writeFile(position,id,file, fileKey, fileType, outputStream,callBack);//上传文件
            }else if(fileList != null){
                for (File f : fileList){
                    writeFile(position,id,f, fileKey, fileType, outputStream,null);
                }
            }else if(fileMap != null){
                for (String key : fileMap.keySet()){
                    writeFile(position,id,fileMap.get(key), key, fileType, outputStream,null);
                }
            }
     ...
     * 上传文件时写文件
     */
    private void writeFile(int position,int id,File file, String fileKey, String fileType, DataOutputStream outputStream, final CallBackUtil callBack) throws IOException {
        ...
        while ((length = inputStream.read(buffer)) != -1){
            ...
            if(callBack != null){
                ...
                CallBackUtil.mMainHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        Log.d("qlf","finalSum="+finalSum);
                        callBack.onProgress(finalSum * 100.0f / total,total,position,id);
                    }
                });
            }
        }
    ...
    private RealResponse getRealResponse(HttpURLConnection conn) throws IOException {
        ...
        Log.d("qlf","getContentLength="+conn.getContentLength());
