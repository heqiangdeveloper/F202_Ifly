...
public class StorageUtil {
    ...
    public static boolean isUsbExit(Context context) {
        File storage = new File(AppConstant.usbRoot).getParentFile();
        if (storage.getName().contains("udisk")) {
            if (isPathExit(storage.getAbsolutePath(),context)) {
                return true;
            }
        } else {
            ...
            if (files != null) {
                for (File file : files) {
                    if (file.getName().contains("udisk") && isPathExit(file.getAbsolutePath(),context)) {
                        return true;
                    }
                }
            }
    ...
    public synchronized static boolean isPathExit(String path,Context context) {
    ...
    public static List<String> getCanWriteUdisk(Context context) {
    ...
    public static String getUuid(String udisk,Context context) {
